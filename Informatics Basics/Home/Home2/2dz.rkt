...
(define (string-trim-right str)
  (let ((sl-tail (string->list str)))
    (define (loop sl-head sl-tail ws-buffer) ; создали буфер для пробельных символов
      (cond ((null? sl-tail) sl-head) ; если строка закончилась
            ...
            (else ; иначе, если мы встречаем просто символ
             (loop (append sl-head ws-buffer (list (car sl-tail)))
                   (cdr sl-tail)
                   '()))))
...
(define (make-multi-vector sizes . fill)
  (list 'multi-vector sizes (make-vector (size-count 1 sizes) fill)))
