    ...
    (if (>= index (vector-length program))
        ...
        (let ((lexem (vector-ref program index)))
          (cond ((number? lexem)
                ...
                ((eq? lexem 'if)
                 (if (not (eq? (car stack) 0))
                     (main (+ 1 index) (cdr stack) dict stack_return)
                     (main (find-endif index) (cdr stack) dict stack_return)))
                ((eq? lexem 'endif) (main (+ 1 index) stack dict stack_return))
                ((eq? lexem 'end) (main (car stack_return) stack dict (cdr stack_return)))
                ((eq? lexem 'exit) (main (car stack_return) stack dict (cdr stack_return)))
                ((not (eq? (assq lexem dict) #f))
